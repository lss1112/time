...
# 創建一個進度條並進行實時更新來模擬數據加載的過程
bar = st.progress(0)
for i in range(100):
    bar.progress(i + 1, f'目前進度 {i+1} %')  # 更新進度條的進度
    time.sleep(0.05)  # 暫停一小段時間模擬載入過程
bar.progress(100, '載入完成！')  # 指示載入已完成
...
start_date = st.sidebar.date_input('模型開始日期', value=dt.date(2020, 1, 1), format="YYYY-MM-DD")
end_date = st.sidebar.date_input('模型結束日期', value=dt.date(2021, 12, 31), format="YYYY-MM-DD")
start_date1 = st.sidebar.date_input('預測開始日期', value=dt.date(2022, 1, 1), format="YYYY-MM-DD")
end_date1 = st.sidebar.date_input('預測結束日期', value=dt.date(2023, 12, 31), format="YYYY-MM-DD")
...
vol_options = st.sidebar.multiselect('選擇波動性', ['Constant', 'GARCH', 'ARCH', 'EGARCH', 'FIGARCH', 'APARCH', 'HARCH'], default=['GARCH'])
